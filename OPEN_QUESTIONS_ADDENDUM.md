...
## Update 2026-09-22 — лиды, ресепшен и снятие прав с директора

Решения владельца (2026-09-22):

| Тема | Решение | Что сделано |
|---|---|---|
| Форма ресепшена | «В ресепшене будут стоять только 2 человека, и они будут давать форму для заполнения» | Новая роль `reception` с обычным входом по коду. Форма живёт по отдельному адресу внутри CRM: публичной ссылки без входа нет, поэтому каждая запись привязана к человеку и попадает в журнал. Аккаунты ресепшена создаёт директор. |
| Редактирование у директора | «Просто уберите» | У директора осталось только чтение, отчётность (включая `POST /daily-reports/regenerate`) и работа с аккаунтами. Снято: создание и правка броней и договоров, отметка взноса, загрузка файла договора, удаление брони и договора, выходные, удаление операций. |
| Правка и удаление смены | Убрать, никому не передавать | Маршруты `PATCH /shifts/{id}` и `DELETE /shifts/{id}` удалены из API, методы сервиса удалены вместе с ними. |
| Удаление начисления зарплаты | Убрать, никому не передавать | Маршрут `DELETE /payroll/entries/{id}` удалён. Черновик правится через `PATCH`, подтверждённое начисление остаётся в истории. |
| Зарплата в отчётности | «Только директору» | `GET /payroll/summary?period=YYYY-MM` — итоги по фонду, налогу, штрафам и прогулам плюс построчные начисления. Роль строго `director`. |
| Работа с лидами | «Начальник распределяет» | Ресепшен заводит лид → начальник продаж назначает менеджера своей команды (`PATCH /leads/{id}/assign`) → менеджер превращает в бронь (`POST /leads/{id}/convert`). Менеджер видит только назначенные ему. |

### Риски, которые создают эти решения

| # | Риск | Почему |
|---|---|---|
| R4 | **Ошибку в смене исправить нечем.** Сотрудник открыл смену по ошибке или прогул записан неверно — изменить запись не может никто, а прогулы влияют на штрафы в зарплате. | Правка смены была только у директора и убрана по решению владельца. Если это начнёт мешать, право придётся вернуть отдельным решением. |
| R5 | **Ошибочное подтверждённое начисление остаётся навсегда.** Удаления нет, а подтверждённое начисление не правится. | Удаление было только у директора. Обходной путь — новая корректирующая запись, но её в ТЗ нет. |
| R6 | **Удалить операцию в закрытом месяце нельзя никому.** Бухгалтеру закрытый месяц запрещает правки, у директора прав больше нет. | Раньше последней инстанцией был директор. |

### Новые вопросы (нужен ответ владельца)

| # | Вопрос | Временное поведение в коде |
|---|---|---|
| C4 | Видят ли два сотрудника ресепшена записи друг друга? | Нет: каждый видит только свои. Выбрано более закрытое поведение. |
| C5 | Может ли лид завести кто-то, кроме ресепшена (менеджер принял звонок на мобильный)? | Нет, только ресепшен. |
| C6 | Нужно ли удалять лиды (ошибочная запись, дубль) и кому? | Удаления нет. Ошибочный лид закрывается статусом «отказ». |
| C7 | Нужен ли лиду срок ответа (например, перезвонить за сутки) и напоминание? | Нет, срок не отслеживается. |
| C8 | Ресепшен заполняет форму сам со слов клиента или отдаёт устройство клиенту? От этого зависит, нужен ли режим киоска, где нельзя уйти с формы. | Форма — обычная страница CRM под аккаунтом ресепшена. |
