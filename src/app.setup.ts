    ...
    new DocumentBuilder()
      ...
      .addTag(
        ...
        'Приходы и расходы. Бэкенд НЕ создаёт операции автоматически: отметка взноса по договору и подтверждение зарплаты ' +
          ...
          'Права: создание, изменение, удаление и вложения — только бухгалтер и только по своим записям в незакрытом месяце. ' +
          'Директор операции видит, но не трогает: решением владельца 22.09.2026 у него осталась лишь отчётность и создание аккаунтов. ' +
          'Начальник продаж и менеджер доступа не имеют; инвестор видит только агрегаты в GET /dashboard.',
      )
      .addTag(
        'leads',
        'Обращения с ресепшена — первый шаг воронки лид → бронь → договор. Регистрирует ресепшен (форма с именем, фамилией, телефоном и нужной площадью); ' +
          'начальник продаж назначает менеджера своей команды; менеджер превращает лид в бронь, добавляя паспорт и подтверждение согласия покупателя. ' +
          'Видимость: ресепшен — свои записи, менеджер — назначенные ему, начальник продаж — нераспределённые и команду, директор — все, только чтение.',
      )
